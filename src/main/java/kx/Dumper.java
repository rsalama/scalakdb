...
public class Dumper
{
    ...
    public static void dump(Object obj, Writer wr) throws Exception
    {
        if (obj instanceof c.Dict)
            dump((c.Dict) obj, wr, rowLimit, colLimit);
        else if (obj instanceof c.Flip)
        {
            dump( ((c.Flip) obj).toTable(), wr, rowLimit, colLimit );
        }
        else if (obj instanceof c.Table)
        {
            dump( (c.Table) obj, wr, rowLimit, colLimit);
        }
        else if (obj.getClass().isArray())
        {
            Object[] arr = c.autoboxArray(obj);
    ...
    private static void dump(c.Dict d, Writer wr, int rowLimit, int colLimit) throws Exception
    {
        if (d.x instanceof c.Flip && d.y instanceof c.Flip)
        {
            // Keyed table
            c.Flip fx = (c.Flip) d.x;
            c.Flip fy = (c.Flip) d.y;
        ...
        else
        {
            ...
            for (Map.Entry<Object, Object> e : m.entrySet())
            {
                if (e.getValue() instanceof c.Flip || e.getValue() instanceof c.Dict)
                {
                    wr.write(e.getKey() + ":\n");
                    dump(e.getValue(), wr);
                }
    ...
    private static void dump(c.Table table, Writer wr, int rowLimit, int colLimit) throws Exception
    {
    ...
    private static String dump(c.Flip f, Writer wr, int rowLimit, int colLimit) throws Exception
    {
        int colWidth[] = new int[colLimit];
    ...
    private static int getRowCount(c.Flip f, int idx)
    {
        int rowCount = 1;
    ...
    private static int calcColWidth(c.Flip f, int colIdx, int rowLimit)
    {
